...
private fun parseQueryPlan(cleaned: String, correlationId: String?): SearchQueryPlan? {
    ...
    if (jsonElement != null) {
        ...
        EventLog.warning(TAG_LLM, "Query plan not JSON, using raw text as single query",
            "Raw: ${fenceStripped.take(AppResources.search.logPreviewMaxChars)}", correlationId = correlationId)
    ...
    val queries = rawQueries
        ...
        .take(3)
        .map { truncateAtWordBoundary(it, AppResources.search.queryMaxChars) }
...
interface LlmClient {
    ...
    suspend fun generateSearchQuery(userMessage: String, summaries: List<Summary> = emptyList(), correlationId: String? = null): SearchQueryPlan? {
        ...
        EventLog.debug(TAG_LLM, "Query plan parsed", "Queries: ${plan.queries.joinToString(" | ")}\nRecency: ${plan.recency}", correlationId = correlationId)
        return plan
    }
