...
- Web search: query generation returns a plan of 1-3 queries + a recency hint, fanned out in parallel by `WebSearchHelper` and merged; parsing is deliberately lenient for small (~9B) models, but an unusable response (`search: false`, non-JSON prose, answer-shaped query strings, truncated output) **skips the search** rather than ever using prose as a query. The query-gen call is non-streaming and surfaces `finish_reason` + HTTP error bodies. When an image is attached and the active model is multimodal, the image is included in query generation so queries are grounded in its content. Planned queries are surfaced as `ConversationEvent.SearchQueriesPlanned` (prompt-plan callback before fan-out; `web_search` tool-call args in agentic mode) and rendered as query pills on the assistant bubble (live in agentic mode) and as `Queries: a | b` in the CLI. See `docs/web-search.md` for the full workflow
- **Reasoning levels** (runtime-only; Android chat bar + CLI `/reasoning`): `ReasoningEffort` is `{ LOW, HIGH, DEEP }`, cycled by `ReasoningEffort.cycle()` / `LlmClient.toggleReasoning(current, webSearchEnabled, agenticMode)` (`null→HIGH→DEEP→LOW`). `DEEP` ("deep research") is reachable only when web search **and** agentic mode are both on; toggling either off demotes `DEEP→HIGH`. `null` = provider default. Provider clients alias `DEEP` to `HIGH` (identical wire params) — only the agentic search-round cap differs. Legacy mode treats `DEEP` as `HIGH` (no round concept). The chat bar shows a fixed-width colored text label (`Low`/`High`) for LOW/HIGH — bright `SendButton` for HIGH, dim `SecondaryIcons` for LOW, and the provider-default `null` state rendered as the dim `Low` label — and `Icons.Filled.AllInclusive` for DEEP; both states occupy a fixed 44dp slot so the spacing to the send button stays constant. A Toast naming the level appears only for DEEP (~1s). The control is unclickable while that Toast is visible, so rapid taps cannot skip levels. Caps come from `agentic.xml` (`agent_standard_search_rounds`=2, `agent_deep_research_search_rounds`=5) via `AgenticConfig`; `AgentTurn` computes the cap, enforces it with a per-turn counter in `WebSearchTool` (over-cap calls return an error result instead of searching) and appends a one-line hint to the tool instruction; `agent_max_steps`=6 stays the outer step bound. `OpenAICompatibleModel` forwards the effort into the agentic request config (previously dropped)
- **Agent thoughts** (agentic mode): the model's reasoning renders in a collapsible `Thoughts` bubble on the assistant bubble (styled like the code-segment bubbles: arrow-icon toggle, copy button, word count, fade-gradient preview, streaming glow) instead of leaking into the answer. It renders markdown via `MarkdownRenderer` once reasoning stops (`thoughtsStreaming` is false) and raw text via `RawTextRenderer` while deltas arrive (avoids re-parsing Markwon per token). Two sources feed it: provider reasoning deltas (`delta.reasoning_content`/`delta.reasoning`) parsed by `reasoningTextFromDelta` and emitted as ADK thought parts (`Part(thought = true)`, excluded by `contentText()` and stripped from replayed history in `OpenAICompatibleModel.appendContent`), and inline `<think>...</think>` blocks in streamed text split by `ThinkTagSplitter` in `AgentTurn`. Both become `AgentTurnEvent.ThoughtsDelta` → `ConversationEvent.ThoughtsDelta` → `ChatMessage.agentThoughts` (+ UI-only `thoughtsStreaming`); pre-tool prose is still retracted via `AnswerReset`. `agentThoughts` is UI-only (never persisted), so the bubble is empty after resume. Non-reasoning models simply produce no thoughts
- **Clarifying questions** (agentic mode): the `ask_user` tool (`AgentTools.kt`) pauses a turn to ask the user a question. The tool suspends on a `QuestionBridge` (`agent/QuestionBridge.kt`, a `Channel` with one buffered slot); the ADK runner awaits tool execution inline, so the turn flow stays open instead of ending — no second invocation or resume protocol. `AgentTurn` always registers the tool (alongside `web_search`/`fetch_url`); its description and instruction come from `agentic.xml` (`ask_user_tool_description` / `agentic_ask_user_instruction`). `ConversationEngine` maps the tool-call args to `ConversationEvent.QuestionAsked(question, options)` (lenient options parsing, max 4), exposes `answerPendingQuestion(text)` (a blank/absent answer is a skip and yields a sentinel to the model), appends the answer to engine history as a USER message on `ToolCallFinished`, and stamps `questionAsked` on the assistant history message so replay formatters include it. Android holds `ChatUiState.pendingQuestion` (`sendMessage()` routes typed input to the answer), renders the question as a standalone assistant bubble with the model's suggested answers as full-width tappable pill rows (one per line, max 4 + a "Skip" row) directly beneath it, and renders the user's reply as a normal full-size user bubble between the question and the response (no card, no mini-bubble); in such Q&A turns any web-search/skipped pills render below the user's answer and above the response segments (chronological order), while normal turns keep them at the top of the bubble; the pills are passed into `MessageRow` only for the asking message while the question is pending (`ChatScreen` gates on `questionAsked != null && questionAnswer == null`), so they disappear once answered. It persists `questionAsked`/`questionAnswer` in `ChatMessage`/`SessionMessage` (unlike UI-only `agentThoughts`); `submitAnswer` records the reply on the asking assistant message instead of appending a separate user message, so the answer remains above the streamed response, and `withRestoredSession` re-inserts the answer as a USER engine history message before the assistant one (plus a migration that folds legacy trailing answer messages). The skip sentinel is `ASK_USER_SKIP_ANSWER` (`AgentTools.kt`) and renders as `(skipped)`. The CLI prompts inline in its event loop. Legacy (non-agentic) mode has no such tool.
