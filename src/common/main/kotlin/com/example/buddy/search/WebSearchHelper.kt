...
class WebSearchHelper(
    ...
    suspend fun search(
        ...
        onQueriesPlanned: (suspend (List<String>) -> Unit)? = null
    ): WebSearchOutcome {
        ...
        return try {
            ...
            onQueriesPlanned?.invoke(plan.queries)
            searchQueries(webSearch, plan.queries, plan.recency, correlationId)
    ...
    companion object {
        // Runs the fan-out/merge for an explicit query list. Used both by the prompt-plan path
        // above and by the agentic `web_search` tool, where the model already chose the queries.
        suspend fun searchQueries(
            ...
            val normalized = queries.map { it.trim() }.filter { it.isNotBlank() }.distinctBy { it.lowercase() }
            ...
            return try {
                ...
                val attempts: List<Pair<String, Result<SearchResponse>>> = coroutineScope {
                    normalized.map { q ->
                        async {
                            q to try {
                                Result.success(webSearch.search(q, recency))
                            } catch (e: CancellationException) {
                                throw e
                            } catch (e: Exception) {
                                Result.failure(e)
                ...
                val succeededQueries = perQuery.map { it.first }
                ...
                Log.info(
                    TAG,
                    "Search completed",
                    "Queries succeeded: ${perQuery.size}/${normalized.size}\nResults: ${results.size}\nAnswer: ${answer?.take(AppConfigProvider.current.search.logPreviewMaxChars) ?: "<none>"}\nPreview: ${resultsText.take(AppConfigProvider.current.search.logPreviewMaxChars)}${if (resultsText.length > AppConfigProvider.current.search.logPreviewMaxChars) "..." else ""}",
                    correlationId = correlationId
                )

                if (results.isEmpty()) {
                    Log.warning(TAG, "Search returned no results", "Queries: ${succeededQueries.joinToString(" · ")}")
                    WebSearchOutcome(queries = succeededQueries, errorMessage = "Web search returned no results")
                } else {
                    WebSearchOutcome(rawResults = results, resultsText = resultsText, answer = answer, queries = succeededQueries)
                }
            } catch (e: CancellationException) {
                throw e
            } catch (e: Exception) {
