...
open class OpenAIClient internal constructor(
    ...
    override fun streamCompletion(messages: List<LlmMessage>, model: String, config: LlmGenerationConfig): Flow<String> = flow {
        ...
        while (true) {
            try {
                ...
                retryCount++
                Log.warning(TAG, "Stream error (attempt $retryCount)", e.message)
                if (retryCount <= 2) {
    ...
    protected open fun detectMultimodalFromApi(modelObj: JsonObject): Boolean {
        try {
            ...
            Log.warning(TAG, "Error parsing modality from API", e.message)
        }
        return false
    }
    ...
    override suspend fun getModels(): List<LlmModel> {
        return withContext(Dispatchers.IO) {
            try {
                ...
                httpClient.newCall(request).execute().use { response ->
                    ...
                    val data = when {
                        ...
                        element.isJsonObject && element.asJsonObject.has("data") ->
                            ...
                            Log.warning(TAG, "Unexpected /models response format")
                ...
                Log.error(TAG, "Failed to acquire model list", e.message)
                emptyList()
            }
        }
    }
    ...
    override suspend fun testConnection(): Boolean {
        return withContext(Dispatchers.IO) {
            try {
                ...
                Log.info(TAG, "Connection check", data = url)
                val request = Request.Builder()
                    .url(url)
                    .header("Content-Type", "application/json")
                    .get()
                ...
                httpClient.newCall(request).execute().use { response ->
                    ...
                    Log.info(TAG, "Connection check completed (code: $code)")
                    response.isSuccessful
                }
            } catch (e: Exception) {
                Log.error(TAG, "Connection check failed", e.message)
                false
            }
        }
    }
            ...
            try {
                ...
                val requestBody = JsonObject().apply {
                    ...
                    add("messages", JsonArray().apply {
                        ...
                        val limitNote = "\n\nEach query must be under ${AppConfigProvider.current.search.queryMaxChars} characters."
                        val systemContent = if (summaries.isNotEmpty()) {
                            dateNote + AppConfigProvider.current.search.queryPrompt + limitNote + "\n\n" + AppConfigProvider.current.summaries.formatSummariesContext(summaries)
                        } else {
                            dateNote + AppConfigProvider.current.search.queryPrompt + limitNote
                        }
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                val requestBody = JsonObject().apply {
                    ...
                    addProperty("temperature", AppConfigProvider.current.summaries.temperature)
                    addProperty("max_tokens", AppConfigProvider.current.summaries.maxTokens)
                ...
                Log.debug(TAG, "Summary generation request", correlationId = null)

                val content = executeStreamingRequest(requestBody, coroutineContext[Job])
                if (content == null) {
                    Log.warning(TAG, "Failed to generate summary")
                ...
                try {
                    ...
                    Log.warning(TAG, "Failed to parse summary JSON", e.message)
                    return@withContext Summary(question = userQuestion, points = emptyList())
                }
            } catch (e: CancellationException) {
                throw e
            } catch (e: Exception) {
                Log.error(TAG, "Failed to generate summary", e.message)
    ...
    override suspend fun compressSummaries(summariesToCompress: List<Summary>, model: String?): Summary {
        return withContext(Dispatchers.IO) {
            try {
                ...
                val prompt = AppConfigProvider.current.prompts.compressSummaries.format(AppConfigProvider.current.summaries.minPoints, AppConfigProvider.current.summaries.maxPoints, summariesText)
                ...
                Log.debug(TAG, "Summary compression request", "Compressing ${summariesToCompress.size} summaries")

                val content = executeStreamingRequest(requestBody, coroutineContext[Job])
                if (content == null) {
                    Log.warning(TAG, "Failed to compress summaries")
                ...
                try {
                    ...
                    val sanitized = AppConfigProvider.current.summaries.sanitizeSummaryPoints(combined)
                    Log.debug(TAG, "Summary compression completed",
                    ...
                    Log.warning(TAG, "Failed to parse compressed summary JSON", e.message)
                    return@withContext Summary(question = "Earlier conversation", points = keyPoints)
                }
            } catch (e: CancellationException) {
                throw e
            } catch (e: Exception) {
                Log.error(TAG, "Failed to compress summaries", e.message)
