...
class AndroidResourcesParityTest {
    ...
    @Test
    fun providersConfigMatchesAndroidResources() {
        assertMatches(
            "providers",
            listOf(
                match("llm_provider_ids", values.arrays["llm_provider_ids"], config.providers.llm.map { it.id }),
                match("llm_provider_names", values.arrays["llm_provider_names"], config.providers.llm.map { it.name }),
                match("llm_provider_urls", values.arrays["llm_provider_urls"], config.providers.llm.map { it.baseUrl }),
                match("websearch_provider_ids", values.arrays["websearch_provider_ids"], config.providers.webSearch.map { it.id }),
                match("websearch_provider_names", values.arrays["websearch_provider_names"], config.providers.webSearch.map { it.name }),
                match("websearch_provider_urls", values.arrays["websearch_provider_urls"], config.providers.webSearch.map { it.baseUrl })
            )
        )
    }
    ...
    private fun resourceDirectory(): File {
        ...
        val candidates = listOf(
            workingDirectory.resolve("../src/android/main/res/values"),
            workingDirectory.resolve("src/android/main/res/values")
