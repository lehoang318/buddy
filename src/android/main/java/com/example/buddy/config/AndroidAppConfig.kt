...
class AndroidAppConfig(context: Context) : AppConfig {
    ...
    override val providers = object : ProvidersConfig {
        override val llm: List<LlmProvider>
            get() {
                val ids = res.getStringArray(R.array.llm_provider_ids)
                val names = res.getStringArray(R.array.llm_provider_names)
                val urls = res.getStringArray(R.array.llm_provider_urls)
                return ids.indices.map { LlmProvider(ids[it], names[it], urls[it]) }
            }

        override val webSearch: List<WebSearchProvider>
            get() {
                val ids = res.getStringArray(R.array.websearch_provider_ids)
                val names = res.getStringArray(R.array.websearch_provider_names)
                val urls = res.getStringArray(R.array.websearch_provider_urls)
